...
### BEGIN INIT INFO
# Provides:          eureka-server
# Required-Start:    $remote_fs $syslog $network
# Required-Stop:     $remote_fs $syslog $network
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Short-Description: eureka-server
...
initEnv() {
  ...
  if [[ -n "$env" ]]; then
    if [[ $env == "PROD" ]]; then
      export ENV=PRO
      export JAVA_OPTS="$JAVA_OPTS -Deureka.client.serviceUrl.defaultZone=http://172.27.3.131:8001/eureka,http://172.27.2.230:8001/eureka,http://172.27.2.202:8001/eureka"
    elif [[ $env == "DEV" ]]; then
      export ENV=DEV
    else
